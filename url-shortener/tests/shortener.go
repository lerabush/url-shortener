...
func TestShortener_success(t *testing.T) {
	...
	client := httpexpect.Default(t, newUrl.String())

	client.POST("/url").WithJSON(save.Request{
		URL:   gofakeit.URL(), // Генерируем случайный URL
		Alias: save.NewRandomString(10),
	}).
...
func testRedirect(t *testing.T, alias string, urlToRedirect string) {
	...
	redirectedToURL, err := shortener.GetRedirect(u.String())
